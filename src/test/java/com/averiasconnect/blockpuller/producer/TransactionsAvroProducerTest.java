...
class TransactionsAvroProducerTest {

  @Mock private KafkaSender<String, TransactionRecord> avroKafkaSender;


  @InjectMocks private TransactionsAvroProducer transactionsAvroProducer;

  @BeforeEach
  void setUp() throws NoSuchFieldException, IllegalAccessException {
    MockitoAnnotations.openMocks(this);
    // Use reflection to set the private transactionsTopic field
    Field topicField = TransactionsAvroProducer.class.getDeclaredField("transactionsTopic");
    topicField.setAccessible(true); // Make the private field accessible
    topicField.set(transactionsAvroProducer, "avro.transactions"); // Set the value
  }
  ...
  void sendTransactions() {
    ...
    // When
    Mono<Long> result = transactionsAvroProducer.sendTransactions(List.of(tx1, tx2, tx3));
  ...
  void sendTransactions_withError() {
    ...
    // When
    Mono<Long> result = transactionsAvroProducer.sendTransactions(List.of(tx1));
        ...
        .expectErrorMatches(
            throwable ->
                throwable instanceof TransactionsProducerException
                    && throwable.getMessage().contains("Error in TransactionsAvroProducer"))
        .verify();
  }
}
